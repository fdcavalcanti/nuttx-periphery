...
class Timer(CharacterDevice):
    ...
    def set_notification_raw(self, payload: bytes | bytearray | memoryview) -> None:
        """Send a raw timer_notify_s payload via TCIOC_NOTIFICATION.

        The timer_notify_s struct embeds struct sigevent whose layout depends
        on NuttX config (e.g. CONFIG_SIG_EVTHREAD), so the caller must pack
        the payload to match the kernel ABI of the target build.
        """
        if not isinstance(payload, (bytes, bytearray, memoryview)):
            raise TypeError("payload must be bytes, bytearray, or memoryview")
        self.ioctl_raw(TCIOC_NOTIFICATION, payload)
